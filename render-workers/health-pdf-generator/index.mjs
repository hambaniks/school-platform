...
async function run() {
  ...
  const { data: alerts } = await supabase.from("clinical_alerts").select("school_id, severity, status, chronic_flag").gte("created_at", thirtyDaysAgo);
  ...
  for (const a of alerts) {
    ...
    summary[a.school_id].total++; if (a.chronic_flag) summary[a.school_id].chronic++;
    if (a.severity === "critical") summary[a.school_id].critical++;
    if (a.status === "resolved") summary[a.school_id].resolved++;
  }
  const report = { generated_at: new Date().toISOString(), period: "monthly", total_alerts: alerts.length, schools: Object.values(summary), email_recipient: healthEmail };
  const fileName = `health-report-${new Date().toISOString().split("T")[0]}.json`;
  const { error } = await supabase.storage.from("official-reports").upload(fileName, JSON.stringify(report, null, 2), { contentType: "application/json", upsert: true });
